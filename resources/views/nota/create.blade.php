...
@push('styles')
    <style>
        /* Enhanced input sizing & visual polish for Nota create */
        .form-card {
            background: #fff;
            padding: 20px;
            border-radius: 12px;
...
@section('content')
    <div class="container">
        ...
        @if (session('status'))
            <div class="alert alert-success"
                ...
                @if (session('status') == 'created')
                    <i class="fas fa-check-circle"></i> Nota berhasil dibuat!
                @endif
            </div>
        @endif
        ...
        <div class="form-card">
            <div class="form-header">
                <h2><i class="fas fa-receipt"></i> Buat Nota Baru</h2>
                <a href="{{ route('nota.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
            </div>

            <form action="{{ route('nota.store') }}" method="POST" id="notaForm">
                @csrf

                <div class="form-row">
                    <div class="form-group">
                        <label for="no">No Nota (opsional)</label>
                        <input type="text" id="no" name="no" placeholder="Kosongkan untuk auto-generate"
                            value="{{ old('no', $no) }}" @if (auth()->check() && auth()->user()->isUser()) readonly @endif>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        @if (auth()->check() && auth()->user()->isUser())
                            {{-- Regular user: select their toko (if any) and make readonly --}}
                            <select id="toko_selection" class="toko-select" disabled>
                                @if ($toko_list->count())
                                    @foreach ($toko_list as $toko)
                                        <option value="{{ $toko->id }}" data-nama="{{ $toko->nama_toko }}"
                                            data-alamat="{{ $toko->alamat }}" selected>{{ $toko->nama_toko }}</option>
                                    @endforeach
                                @else
                                    <option value="">-- Tidak ada toko terdaftar --</option>
                                @endif
                            </select>
                            <input type="hidden" id="toko_id" name="toko_id"
                                value="{{ optional($toko_list->first())->id }}">
                        @else
                            <select id="toko_selection" class="toko-select">
                                <option value="">-- Pilih Toko --</option>
                                @foreach ($toko_list as $toko)
                                    <option value="{{ $toko->id }}" data-nama="{{ $toko->nama_toko }}"
                                        data-alamat="{{ $toko->alamat }}">{{ $toko->nama_toko }}</option>
                                @endforeach
                                <option value="manual">+ Tulis Manual</option>
                            </select>
                            <input type="hidden" id="toko_id" name="toko_id" value="{{ old('toko_id') }}">
                        @endif
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="nama_toko" name="nama_toko" placeholder="Masukkan nama toko"
                            value="{{ old('nama_toko', optional($toko_list->first())->nama_toko) }}" required
                            @if (auth()->check() && auth()->user()->isUser()) readonly @endif>
                        <input type="hidden" id="nama_toko_manual" name="nama_toko_manual" value="">
                    </div>
                </div>
                ...
                <div class="form-row full">
                    <div class="form-group">
                        ...
                        <textarea id="alamat" name="alamat">{{ old('alamat', optional($toko_list->first())->alamat) }}</textarea>
                        <input type="hidden" id="alamat_toko_manual" name="alamat_toko_manual" value=""
                            @if (auth()->check() && auth()->user()->isUser()) readonly @endif>
                    </div>
                </div>
                        ...
                        <div>
                            @if (auth()->check() && auth()->user()->isAdmin())
                                <label style="font-weight:600;">
                                    <input type="checkbox" name="update_harga" value="1"> Perbarui harga master
                                    barang
                                </label>
                            @endif
                        </div>
            ...
            @if (auth()->check() && auth()->user()->isAdmin())
                <div id="addBarangModal" class="modal-overlay"
                    ...
                    <div class="modal-dialog"
                        ...
                        <h3 style="margin:0 0 8px;"><i class="fas fa-plus-circle"
                                style="color:#2ecc71;margin-right:8px"></i>
                            Tambah Barang Baru</h3>
                        <form id="addBarangForm" onsubmit="event.preventDefault(); submitNewBarang();"
                            ...
                            <div style="grid-column:1 / -1;">
                                ...
                                <div id="uraianError" style="color:#c0392b;display:none;margin-top:6px;font-size:0.9rem">
                                </div>
                            </div>
    ...
    @push('scripts')
        <script>
            // Permission flags for client-side logic
            window.canCreateMasterBarang = @json(auth()->check() && auth()->user()->isAdmin());

            const barangList = {!! json_encode(
                $barang_list->map(function ($b) {
                    return [
                        'id' => $b->id,
                        'uraian' => $b->uraian,
                        'satuan' => $b->satuan,
                        'harga_satuan' => $b->harga_satuan,
                        'profit_per_unit' => $b->profit_per_unit ?? 0,
                    ];
                }),
            ) !!};
            const satuanList = {!! json_encode($satuan_list->map->nama_satuan) !!};

            // If regular user and has a single toko, prefill toko fields on load
            document.addEventListener('DOMContentLoaded', function() {
                @if (auth()->check() && auth()->user()->isUser() && $toko_list->count())
                    (function() {
                        const toko = @json($toko_list->first());
                        if (toko) {
                            document.getElementById('toko_id').value = toko.id || '';
                            const sel = document.getElementById('toko_selection');
                            if (sel) sel.value = toko.id;
